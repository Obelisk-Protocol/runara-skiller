...
router.post('/fetch-player-cnfts-simple', async (req: any, res: any) => {
  try {
    ...
    console.log('📊 [CNFT Route] Profile query result:');
    console.log('- Profile data:', profile);

    if (!profile) {
      console.error('❌ [CNFT Route] Failed to fetch profile');
...
router.post('/mint-item', async (req: any, res: any) => {
  try {
    ...
    const profileResult = await pgQuerySingle<any>(
      'SELECT id, character_name, character_class, player_pda FROM profiles WHERE id = $1',
      [playerId]
    );
