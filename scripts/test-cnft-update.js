 ...
 * Usage:
 *   npm run build
 *   node scripts/test-cnft-update.js <assetId>
 * 
 * Or with tsx (no build needed):
 *   npx tsx scripts/test-cnft-update.ts <assetId>
 * 
 * Example:
 *   node scripts/test-cnft-update.js DQfAMfrSFShguVdMiYfSXuuHaKTnS93TM8JPdW9khUrs
 */
...
const { NftColumns } = require('../dist/services/database');

async function testCNFTUpdate(assetId) {
...
if (!assetId) {
  console.error('❌ Usage: node scripts/test-cnft-update.js <assetId>');
  console.error('   Or: npx tsx scripts/test-cnft-update.ts <assetId>');
  console.error('\nExample:');
  console.error('  node scripts/test-cnft-update.js DQfAMfrSFShguVdMiYfSXuuHaKTnS93TM8JPdW9khUrs');
  process.exit(1);
}
